"""
features_pv21.py — the TWENTY PV / battery main features of pv20_features.py, unchanged, computed only
from the AEW smart-meter CSVs, + feature 21, which also uses the canton-average global irradiance
(MeteoSwiss, Buchs/Aarau + Beznau; built by meteo_aargau.py, which must sit in the same folder).

Run (inside the Renku session, from ~/work/mechenergy):
    NO ARGUMENTS:  python features_pv21.py
    ...
    FAST PATH — after a one-time  python build_store.py ../aew-data/test-blob/input_data -o aew_store :
        python features_pv21.py --store aew_store --mp-ids 53628,45390 -o two_houses.csv     (milliseconds)
        python features_pv21.py --store aew_store -o all_houses.csv                          (all 89k houses, 8 buckets at
                                                             a time so memory stays bounded; --batch-buckets 4 for less)
    ...
    ten houses, every month that exists (one or two whole years):
        python features_pv21.py ../aew-data/test-blob/input_data --max-houses 10 -o ten_houses.csv
    named houses (e.g. ones labelled PV):
        python features_pv21.py ../aew-data/test-blob/input_data --mp-ids 53628,45390,49688 -o pv.csv
    all labelled houses at once (one MP ID per line in the file; one slow pass, then cached):
        python features_pv21.py ../aew-data/test-blob/input_data --mp-ids-file labelled_ids.txt -o labelled.csv
    only 2023:
        python features_pv21.py ../aew-data/test-blob/input_data --max-houses 10 --month 2023
    every house of one month (slow but bounded memory):
        python features_pv21.py ../aew-data/test-blob/input_data --month "Juni 2023" -o juni2023.csv
...
C. IRRADIANCE — needs data/weather/aargau_weather_15min.csv (built automatically on the first
   run if missing: meteo_aargau.py downloads MeteoSwiss data, needs internet, ~1 min)

21. irr_net_slope_kw_summer  [kW; PV > 0, no PV ~ 0]  for every summer day: x = canton irradiation
    9-17 h [kWh/m²], y = import - export 9-17 h [kWh]; least-squares line y = a + b·x over the
    house's summer days; feature = -b.  Panels produce ~ kWp · PR · irradiation, so -b ~ 0.7-0.9 x
    installed kWp.  Sunny vs cloudy DAYS at the same clock hours: the daily routine cancels, the
    panels remain.  No PV / grid-charged battery -> ~0.  Days need >= 90 % of their 9-17 h data;
    < 3 days or no weather variation -> 0.  A meter without export register whose panels cover
    the house on most summer days gives a flat line (small value) -> features 13 / 20 catch it.
...
export_max_raw_kw, n_irr_days (days used by feature 21),
...
# =============================================================================
# DEFAULTS — what  python features_pv21.py  does when run with no arguments.
...
DEFAULT_OUT = "labelled_features_21.csv"    # output table (an .xlsx copy is written next to it)
...
MIN_IRR_DAYS = 3             # feature 21: fewer usable summer days -> 0
MIN_X_STD = 0.1              # feature 21: kWh/m²; days all alike (no weather variation) -> 0
...
def feature_table(days: pd.DataFrame, irr_daily: Optional[pd.Series] = None) -> pd.DataFrame:
    ...
    # ---------------- C. irradiance (irr_daily: kWh/m² 9-17 h per date; None -> NaN, filled with 0) ----------------
    n_irr = pd.Series(0, index=houses)
    if irr_daily is None:
        out["irr_net_slope_kw_summer"] = np.nan
    else:
        okd = Is.loc[Is["n_valid_day"] >= 0.9 * DAY_MASK.sum(), ["mp_id", "date", "day_tot"]]
        okd = okd.merge(Es[["mp_id", "date", "day_tot"]].rename(columns={"day_tot": "exp_day_tot"}),
                        on=["mp_id", "date"], how="left")                  # export of the same day (none -> 0)
        x = pd.to_datetime(okd["date"]).dt.normalize().map(irr_daily)
        y = (okd["day_tot"] - okd["exp_day_tot"].fillna(0.0)) / 4.0        # kWh 9-17 h, import - export
        sel = (x.notna() & y.notna()).values
        xy = pd.DataFrame({"mp_id": okd["mp_id"].values[sel], "x": x.values[sel].astype(float),
                           "y": y.values[sel].astype(float)})
        ...
        good = (n >= MIN_IRR_DAYS) & (den > MIN_X_STD ** 2 * n * n)
        slope = ((n * sums["xy"] - sums["x"] * sums["y"]) / den.where(good, 1.0)).where(good, 0.0)
        out["irr_net_slope_kw_summer"] = R(-slope)
        n_irr = R(n)
    ...
    out["n_irr_days"] = n_irr.astype(int)               # days used by feature 21
    return out
...
FEATURE_DESC = {
    ...
    "irr_net_slope_kw_summer": "-(slope of daily 9-17 h import-export [kWh] vs canton 9-17 h irradiation [kWh/m²]) over summer days ~ 0.7-0.9 x kWp (no PV ~0)",
    ...
    "n_irr_days": "summer days used by feature 21 (>= 90 % of 9-17 h data and known irradiation)",
}
...
def house_features(house: Dict[str, pd.DataFrame], irr_daily: Optional[pd.Series] = None) -> Dict[str, float]:
    parts = []
    ...
    return feature_table(days, irr_daily).iloc[0].to_dict()


# =============================================================================
# 4b. weather for feature 21
# =============================================================================
def load_irradiation(days: pd.DataFrame, path: str = DEFAULT_WEATHER) -> pd.Series:
    """kWh/m² of canton irradiation 9-17 h per date.  Builds the weather file with meteo_aargau.py
    (MeteoSwiss download, needs internet) if it does not exist yet."""
    from meteo_aargau import build, daily_irradiation, load_weather
    ...
    if not os.path.exists(path):
        ...
        try:
            ...
            raise SystemExit(f"could not build the weather file ({e}).  Without internet: download\n"
                             ...
                             f"into {os.path.join(os.path.dirname(path), 'raw')}/ and run  python meteo_aargau.py --start {start} --end {end}")
        if os.path.abspath(built) != os.path.abspath(path):
            os.replace(built, path)
    irr = daily_irradiation(load_weather(path), hours=DAYLIGHT)
    print(f"weather: {len(irr):,} days of 9-17 h irradiation ({irr.index.min().date()} .. {irr.index.max().date()}), "
          f"meter data {start} .. {end}", flush=True)
    return irr
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    ap.add_argument("--workers", type=int, default=DEFAULT_WORKERS, help="files skimmed in parallel when a house list is given (default 4)")
    ...
    ap.add_argument("--batch-buckets", type=int, default=8, help="store, all houses: buckets processed together "
                    "(default 8 of 256; lower = less memory; 0 = all at once as in pv20_features.py)")
    ...
    if store and ids is None and not a.max_houses and a.batch_buckets > 0:
        # all houses: a few buckets at a time -> features -> next buckets (one row per house kept, the days dropped)
        buckets, tabs, irr, t0 = store_buckets(store), [], None, time.time()
        for i in range(0, len(buckets), a.batch_buckets):
            group = buckets[i:i + a.batch_buckets]
            days = load_days(None, verbose=False, month=a.month, mp_ids=store_ids(group), workers=a.workers, store=store)
            if len(days):
                irr = load_irradiation(days, a.weather) if irr is None else irr
                tabs.append(feature_table(days, irr))
            del days
        ...
        tab = feature_table(days, load_irradiation(days, a.weather))
