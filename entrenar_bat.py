...
modelo_bat, X_test, y_test, X_train, y_train = entrenar_modelo(X, y_bat, nombre="bat", max_depth=6)
evaluar_modelo(modelo_bat, X_test, y_test, nombre="bat")

# 4. Diagnóstico: ¿hay overfitting? Comparamos AUC en train vs en test
proba_train = modelo_bat.predict_proba(X_train)[:, 1]
proba_test = modelo_bat.predict_proba(X_test)[:, 1]
