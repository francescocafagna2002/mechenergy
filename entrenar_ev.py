...
modelo_ev, X_test, y_test, X_train, y_train = entrenar_modelo(X, y_ev, nombre="ev", max_depth=6)
evaluar_modelo(modelo_ev, X_test, y_test, nombre="ev")

# 4. Diagnóstico: ¿hay overfitting? Comparamos AUC en train vs en test
proba_train = modelo_ev.predict_proba(X_train)[:, 1]
proba_test = modelo_ev.predict_proba(X_test)[:, 1]
...
explicar_prediccion(modelo_ev, X, mp_id=X.index[0])

guardar_modelo(modelo_ev, "modelo_ev.pkl")